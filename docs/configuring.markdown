...
* `cache_file_format`: The default file format for cached data is 'RData'.  This can be set to 'qs2' in order to benefit from the optimized compression format provided by `qs2`.  For legacy reasons, 'qs' from the `qs` package is still available, but not recommended for new projects.
