...
After editing your tests, you can call `test.project()` to run all of the unit tests in the `tests` directory`[^1]`{=markdown}.
...
`[^1]:`{=markdown}  Some versions of `TestThat` may required that `load.project()` is run before `stub.tests()`
