...
After editing your tests, you can call `test.project()` to run all of the unit tests in the `tests` directory<sup>[1](#testing_footnote)</sup>.
...
<a name="testing_footnote">1</a>: Some versions of `TestThat` may required that
`load.project()` is run before `stob.tests()`
