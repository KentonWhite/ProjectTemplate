...
#' \code{code/tests/autogenerated.R}. Every test is excepted to fail by default,
...
stub.tests <- function()
{
  ...
  autogenerated.path <- file.path('code', 'tests', 'autogenerated.R')

  unlink(autogenerated.path)

  # Source each file in the `lib` directory in a temporary environment.
  lib.files <- dir(file.path('code', 'lib'), pattern = "[.][rR]$")
  tmp.env <- new.env()

  message(paste('Generating stub tests in ', autogenerated.path, ':', sep = ''))

  for (lib.file in lib.files)
  {
    sys.source(file.path('code', 'lib', lib.file), envir = tmp.env)
